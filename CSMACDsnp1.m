...
G=0:1/Step:x_range;
C=zeros(size(G));
C=(a.*G.*exp(-a.*G))./(a.*G.*exp(-a.*G)+a.*g.*(1-exp(-a.*G)-a.*G.*exp(-a.*G))+a.*(2-exp(-a.*G)-a.*G.*(-a.*G)));
...
if get(hold,'value')
   ...
   cla reset                                                                              
   set(A,'Box','on');
   Text=sprintf('Slotted CSMA/CD non-persistent  -  %s = %g[ms], C = %d[kbps], L_f = %d[bit], %s = %d, a = %g','\tau',mtau,c,L,'\gamma',g,a);
   xl='Offered load, G';
   yl='Throughput, S';
...
if get(findobj('Tag','CoordEnable'),'Value')
   ...
   coyres=(a*cox*exp(-a*cox))/(a*cox*exp(-a*cox)+a*g*(1-exp(-a*cox)-a*cox*exp(-a*cox))+a*(2-exp(-a*cox)-a*cox*(-a*cox)));
