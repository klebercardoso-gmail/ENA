...
if get(hold,'value')
   ...
   cla reset                                                                              
   set(A,'Box','on');
   Text=sprintf('Network efficiency  -  Slotted Aloha');
   xl='Propagation delay, a';
   yl='Network efficiency, \eta_S_A';
   tit='Network efficiency';
   plot2(Gg,Al,'r',xl,yl,Text,Text1);
   if get(xyEn,'value')
      axis([0 x_range str2double(get(ym,'String')) str2double(get(y,'String'))]);
   end
end                                                                                                                                                                                                                                                                                         
...
if get(findobj('Tag','CoordEnable'),'Value')
   ...
   elseif get(coy1,'String')==''
      return
   elseif msgbox('Coordinate Y must be: Y = 0.368','Limit Violation','warn','modal');
      return
   end
end
%
