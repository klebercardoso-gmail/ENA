...
Gg=0:1/Step:x_range;
D=zeros(size(Gg));
D=(exp(2*Gg)-1)*((K-1)/2+2*a+1)+1+a;
% Se è spuntato "Hold on"                                                                                                                                                                                                                                                                   
if get(hold,'value')
   D=(exp(2*Gg)-1)*((K-1)/2*a+1)+1+a;                                                                                                                                                                                                                                                
   set(F,'CurrentAxes',A);                                                                                                                                                                                                                                                                 
   set(A,'NextPlot','add');
   set(hold,'Value',1);
   xl='Offered load, G';
   ...
   cla reset                                                                              
   set(A,'Box','on');
   Text=sprintf('Aloha     -     %s = %d[ms], C = %d[kbps], L_f = %d[bit], K = %d, a = %g','\tau',mtau,c,Fr,K,a);
   xl='Offered load, G';
   yl='Average normalized delay, D*_A';
   plot2(Gg,D,'b',xl,yl,Text,Text1);
   if get(xyEn,'value')
      axis([0 x_range str2double(get(ym,'String')) str2double(get(y,'String'))]);
   end
end                                                                                                                                                                                                                                                                                         
