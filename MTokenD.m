...
if get(hold,'value')
   ...
   cla reset                                                                              
   set(A,'Box','on');
   Text=sprintf('Multiple Token   -  C = %d[kbps], L_f = %d[bit], B = %d[bit-time], %s = %d[ms], a* = %g',c,L,B,'\tau',mtau,ap);
   xl='Throughput, S';
   yl='Acerage normalized delay, D*_m_t';
...
if get(findobj('Tag','CoordEnable'),'Value')
   ...
   if cox > max(S) | cox < min(S)
      testo1=sprintf('Coordinate X must be: %g < X < %g',min(S),max(S));
