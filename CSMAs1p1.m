...
G=0:1/Step:x_range;
C=zeros(size(G));
C=(G.*exp(-G.*(1+a)).*(1+a-exp(-a.*G)))./((1+a).*(1-exp(-a.*G))+a.*exp(-G.*(1+a)));
...
if get(hold,'value')
   ...
   cla reset                                                                              
   set(A,'Box','on');
   Text=sprintf('Slotted CSMA 1-persistent    -    %s = %g[ms], C = %d[kbps], L_f = %d[bit], a = %g','\tau',mtau,c,L,a);
   xl='Offered load, G';
   yl='Throughput, S';
...
if get(findobj('Tag','CoordEnable'),'Value')
   ...
   coyres=(cox*exp(-cox*(1+a))*(1+a-exp(-a*cox)))/((1+a)*(1-exp(-a*cox))+a*exp(-cox*(1+a)));
