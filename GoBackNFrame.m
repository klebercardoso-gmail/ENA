...
Wmax=(Ack*(1-W)+2*tau*C)/(W-1);
limite=(Ack*(1-W)+2*tau*C)/(W-1);
Go=zeros(size(L));
Go=W*(1-P)./(1+2*tau*C./(L+Ack))*(1+(W-1)*P);
...
if get(findobj('Tag','CoordEnable'),'Value')
   ...
   coyres=W*(1-P)/(1+2*tau*C/(cox+Ack))*(1+(W-1)*P);
   coy1=findobj('Tag','yCoord');
   coy=str2double(get(coy1,'String'));
   if cox < 0 | cox > (Ack*(1-W)+2*tau*C)/(W-1)
      testo=sprintf('Coordinate X must be: 0 < X <= (L_a(1-W)+2%sC)/(W-1)\n\n (L_a(1-W)+2%sC)/(W-1) = %g','\tau','\tau',limite);
