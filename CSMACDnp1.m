...
if get(hold,'value')
   ...
   cla reset                                                                              
   set(A,'Box','on');
   Text=sprintf('CSMA/CD non-persistent  -  %s = %g[ms], C = %d[kbps], L_f = %d[bit], %s = %d, a = %g','\tau',mtau,c,L,'\gamma',g,a);
   xl='Offered load, G';
   yl='Throughput, S';
...
if get(findobj('Tag','CoordEnable'),'Value')
   ...
   coyres=(cox*exp(-a*cox))/(cox*exp(-a*cox)+cox*a*cox*(1-exp(-a*cox))+2*a*cox*(1-exp(-a*cox))+(2-exp(-a*cox)));
